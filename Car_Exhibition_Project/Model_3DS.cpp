...
// Model_3DS.cpp -> Model_3DS::MapNameChunkProcessor
...
void Model_3DS::MapNameChunkProcessor(long length, long findex, int matindex)
{
	...
	// *******************************************************
	// ** التعديل الجديد: تغيير لاحقة الملف (.jpg إلى .bmp) **

	// 1. نبحث عن النقطة ('.') الأخيرة في اسم الملف (مثال: 'bark_loo.jpg')
	char* dot = strrchr(name, '.');

	if (dot)
	{
		// 2. نستبدل اللاحقة بـ ".bmp"
		// (يجب أن تكون قد حولت صورك يدوياً إلى هذه الصيغة)
		strcpy(dot, ".bmp");
	}
	...
	// هذا السطر سيستخدم المسار الصحيح (من دالة Load) والاسم المُعدّل (bark_loo.bmp)
